...
class TechWatchStore {
    ...
    // حمّل المنتجات من المصفوفة المحلية بدلاً من JSON
    async loadProducts() {
        try {
            // بخبرة محترف: لا نحتاج لـ fetch ولا لـ await هنا لأن البيانات محملة مسبقاً
            // لكن نبقي على هيكل الـ async لضمان عدم كسر باقي أجزاء الكود
            this.products = PRODUCTS_DATA; 
            console.log('✅ Success: Products loaded from local module');
        } catch (error) {
            console.error('CRITICAL ERROR: Failed to assign product data', error);
            this.showToast('Error displaying products', 'error');
        }
    }   
